...
const LeaderBoard = () => {
  ...
  // Update your return statement JSX structure like this:
...
return (
  <div className={`leaderboard-container ${styles.fullScreenDiv}`}>
    ...
    <div className={`${styles.leader_outer}`}>
      <canvas ref={canvasRef} className={styles.canvas} id="canvas" />
      <div className={`${styles.leader_inner}`}>
        ...
        {loggedInUserRank ? (
          <div className={`${styles.rank_outer}`}>
            <div className={`${styles.rank_inner}`}>
          ...
          <div className={styles.rank_outer}>
            <div className={styles.rank_inner}>
              <h2 className="text-xl font-bold text-white mb-2">Your Rank: N/A</h2>
        ...
        <div className={styles.outerRankBoard}>
          ...
          <div className={styles.centerImage}>
            <img src="/leader.svg" alt="Leader" />
            <p>
              Gold, Silver, Bronze: more than medals, they're your hustle goals!
            </p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {users.slice(0, 3).map((user, index) => (
              <div
                key={user.userId}
                className={`${styles.rank_board_outer} border-t-4 border-[#9793ba]`}
              >
        ...
        <div className={styles.leader_board_outer}>
          <div className={`overflow-y-auto ${styles.leader_board_inner}`}>
            <table className="min-w-full relative border-gray-400 shadow-md rounded-lg overflow-hidden hover:cursor-pointer">
              <thead className={`text-md sticky top-0 z-10 font-bold ${styles.table_head}`}>
                <tr>
                  <th className="px-5 py-3.5">Rank</th>
                  <th className="px-5">Username</th>
                  <th className="px-5">Completed</th>
                  <th className="px-5">Score</th>
                  <th className="px-5">Reward</th>
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr
                    ...
                    className={`hover:bg-g hover:cursor-pointer text-sm duration-300 ${
                      user.userId === loggedInUserRank?.userId
                        ? "bg-transparent"
                        : "bg-transparent"
                    } ${styles.leader_table_li}`}
                  >
                    <td className="px-5 py-3 text-gray-300">{index + 1}</td>
                    <td className="px-5 py-3 text-gray-300 hover:scale-105">
                      {user.username}
                    </td>
                    <td className="px-5 py-3 text-gray-300">
                      {user.completedTasks}
                    </td>
                    <td className="px-5 py-3 text-gray-300">{user.score}</td>
                    <td className="px-5 py-3 text-gray-300">
                      {getReward(index)}
                    </td>
                  </tr>
